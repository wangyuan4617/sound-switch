...
- 待办：让用户在不手动启动耳机的前提下开关一次耳机，做后台实例的端到端确认。

## 2026-09-10 23:25 — 第 18 步：后台实例端到端确认 + 便携部署（迁移）支持
### 18.1 后台实例端到端确认（无需手动启动任何程序）
用户直接开关耳机，由**登录自启的隐藏实例**（23:21:17 启动）完成处理：
```
23:25:19 收到设备开机事件 → console: Realtek → 耳机        (需求2)
23:25:36 收到设备关机事件 → console: 耳机 → Realtek        (需求3)
23:25:47 收到设备开机事件 → console: Realtek → 耳机        (需求2)
```
- 结论：**自启实例完全可用**，用户日常不再需要手动开窗口。

### 18.2 便携部署（回答"换电脑/重装系统怎么搬"）
- **依赖检查**：用 dumpbin 检查发现原 exe 依赖 `VCRUNTIME140.dll`（VC++ 运行库），
  全新系统可能缺失 → 新增 `.cargo/config.toml`，启用 `+crt-static` 静态链接 CRT。
  重新构建后 dumpbin 确认依赖只剩系统 DLL（kernel32/ole32/oleaut32/ntdll/bcryptprimitives/
  api-ms-win-core-*），**新机器无需安装 VC++ 运行库**；exe 体积 601KB→731KB（启动器 226KB→325KB）。
- **目录结构兼容**：程序改为同时支持两种布局
  - 便携结构：exe 与 `config.json` 同在根目录；
  - 开发结构：exe 在 `target\release\`。
  `install-autostart.ps1` 自动按 `根目录 → bin\ → target\release\` 顺序查找 exe；
  `launch.rs` 的工作目录回退顺序为 `当前目录 → exe 目录 → 上一级 → 上两级`。
- **新增 `scripts\make-package.ps1`**：生成便携包 `dist\sound-switch-portable\` + `.zip`
  （含两个 exe、config.json、脚本、说明文档、源码、`.cargo\config.toml`；
  不含 `state.json`/`logs\`/`target\`；不含体积较大的实验快照）。
  实测产物：27 个文件 / 1.13 MB，zip 0.53 MB。
- **实测验证**：
  - 从便携包目录运行 `sound-switch.exe list` → 正确识别 3 个 HID 集合与耳机端点；
  - 从便携包目录运行包内启动器 → 日志正确写入**包内** `logs\`（证明工作目录解析正确），
    并因已有实例而被单实例保护拦截（符合预期）；
  - 测试残留（包内 logs/state.json）已清理。
- **新增 `docs\DEPLOY.md`**：完整迁移指南（便携包流程、手工最小文件集、新机器从源码编译、
...
## 2026-09-10 23:50 — 第 20 步：包内加"使用说明.txt"与双击安装/卸载脚本
- 用户反馈两点：① 包里一点文档都没有；② 安装还要手动敲命令。
- **改动**：
  - 新增 `scripts\status.ps1`：一屏显示计划任务/后台进程（含是否无窗口）/最近日志/当前默认输出。
  - `install-autostart.ps1`：新增 `-Pause`（结束时 `Read-Host` 等待回车，避免双击时窗口一闪而过）；
    新增“已存在同名任务但指向别的目录”的警告（移动文件夹后重装时给出提示）；末尾增加中文“安装完成”指引。
  - `uninstall-autostart.ps1`：重写并支持 `-Pause`，输出中文步骤说明。
  - 新增 `packaging\` 目录存放包内根级文件：
    `安装登录自启.bat`、`卸载登录自启.bat`、`查看运行状态.bat`（**纯 ASCII + CRLF**，
    只做参数拼装并调用 `scripts\*.ps1`，中文提示全部由 PowerShell 输出 → 规避 cmd 编码坑），
    以及 `使用说明.txt`（**UTF-8 with BOM**，记事本直接可读；含功能说明/安装/卸载/文件说明/
    常用命令/config 配置项/换机迁移/常见问题 7 节）。
  - `make-package.ps1`：把 `packaging\*` 复制到包根；`scripts\` 增加 `status.ps1`；
    汇总输出改为“双击安装/看状态/卸载”的说明。
- **实测**（解压 zip 到临时目录，安装测试使用自测任务名 `sound-switch-selftest`，**不改动用户已装的任务**）：
  - `查看运行状态.bat`：中文输出正常，能正确报告任务/进程/日志/默认输出；
  - `安装登录自启.bat`：任务创建正确（指向包内启动器、工作目录=包目录）、
    进程立即后台启动且 `MainWindowHandle=0`、日志写入包目录、中文提示与“安装完成”完整显示；
  - `卸载登录自启.bat`：任务删除、进程结束；
  - 用户正式任务 `sound-switch` 未被触碰（仍指向 `<安装目录>`）；
  - 临时目录已清理。
- **发现**：用户已自行把包放到 `<安装目录>` 并注册了任务，但任务
